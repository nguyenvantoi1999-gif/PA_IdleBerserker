...
// 마계 공격력의 n%로 범위 공격 (후방 몬스터 2,3마리)
public class CompanionSkill_16 : CompanionActiveSkill
{
    ...
    protected override void Active(List<CharacterObject> targets)
    {
        ...
        if (diff >= 1)
        {
            // CE OPTION SKILL, 처치한 적의 수 * n% 만큼 마계 피해 증가
            ...
            if (IsCEEquipped())
            {
                ...
                var CESkillOption = new Buff(Enum_BuffFrom.CompanionEquipment, buffID, _specData.skillDuration)
                    .AddStatusEffect(Enum_Good_Status_Effect.StatFireDamageMultiply, (diff * specSubStat.value));
